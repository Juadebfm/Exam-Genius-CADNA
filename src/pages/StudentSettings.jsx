...
import { apiClient, API_ENDPOINTS } from '../config/api'; // adjust path to match your project
...
const Toggle = ({ value, onChange, disabled }) => (
  <button
    ...
    style={{
      ...
      alignItems: 'center', borderRadius: '9999px', border: 'none',
...
const DeleteModal = ({ onClose, onConfirm, loading }) => (
  <div style={{
    ...
    <div style={{ background: '#fff', borderRadius: '12px', padding: '28px', maxWidth: '400px', width: '90%' }}>
      <h3 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '12px', color: '#111827' }}>Delete Account</h3>
...
const ChangePasswordModal = ({ onClose, userId }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: '#fff', borderRadius: '12px', padding: '28px', maxWidth: '400px', width: '90%' }}>
        <h3 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '20px', color: '#111827' }}>Change Password</h3>
        {[
        ...
        {success && <p style={{ color: '#16A34A', fontSize: '13px', marginBottom: '12px' }}>Password changed successfully!</p>}
...
const EditProfileModal = ({ profile, userId, onClose, onSaved }) => {
  ...
  const purposeOptions = [
    'Academic Studies', 'Professional Development', 'Research',
    'Personal Learning', 'Corporate Training', 'Other',
  ];
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', padding: '32px 32px 0' }}>
          ...
          <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#111827', margin: '0 0 4px' }}>User Profile</h2>
          <p style={{ fontSize: '13px', color: '#6B7280', margin: '0 0 24px' }}>View and manage user details</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '32px' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <div style={{
                ...
                fontSize: '32px', fontWeight: '600', color: '#2563EB',
                overflow: 'hidden',
              }}>
                {avatarPreview
        ...
        <div style={{ padding: '0 32px 32px' }}>
          ...
          {/* User Information */}
          <h3 style={{ fontSize: '15px', fontWeight: '600', color: '#111827', margin: '0 0 16px' }}>User Information</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '28px' }}>
            {[
              ...
              <div key={f.name}>
                <label style={{ display: 'block', fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>{f.label}</label>
                <div style={wrapStyle}>
                  ...
                  <input
                    ...
                    placeholder={f.label}
                    style={{ ...fieldStyle, ...(f.readOnly ? { background: '#F9FAFB', cursor: 'not-allowed', opacity: 0.7 } : {}) }}
                  />
                </div>
              </div>
            ))}
          ...
          <h3 style={{ fontSize: '15px', fontWeight: '600', color: '#111827', margin: '0 0 16px' }}>Academic/Professional details</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            {/* Purpose of Use — dropdown */}
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Purpose of Use</label>
              <div style={wrapStyle}>
                <span style={iconStyle}>
            ...
            {/* Institution */}
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Institution</label>
              <div style={wrapStyle}>
                <span style={iconStyle}>
          ...
          <div style={{ marginBottom: '32px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Department</label>
            <div style={wrapStyle}>
              <span style={iconStyle}>
...
const StudentSettings = () => {
  ...
  const [settings, setSettings] = useState({
    twoFactorAuth: false,
    examReminder: false, resultAlerts: false, systemUpdates: false,
    webcamAccess: false, microphoneAccess: false,
  });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setSettings({
          twoFactorAuth:    u.twoFactorEnabled ?? u.twoFactorAuth ?? false,
          examReminder:     prefs.examReminder     ?? false,
          resultAlerts:     prefs.resultAlerts     ?? false,
          systemUpdates:    prefs.systemUpdates    ?? false,
          webcamAccess:     prefs.webcamAccess     ?? false,
          microphoneAccess: prefs.microphoneAccess ?? false,
        });
      } catch (err) {
  ...
  const handleToggle = async (key) => {
    ...
    try {
      if (key === 'twoFactorAuth') {
        ...
        const prefs = JSON.parse(localStorage.getItem('userPrefs') || '{}');
        localStorage.setItem('userPrefs', JSON.stringify({ ...prefs, [key]: newVal }));
      }
    } catch (err) {
  ...
  const handleDarkMode = (val) => {
    setDarkMode(val);
    const prefs = JSON.parse(localStorage.getItem('userPrefs') || '{}');
    localStorage.setItem('userPrefs', JSON.stringify({ ...prefs, darkMode: val }));
  };
  ...
  const labelStyle   = { fontSize: '12px', color: dm ? '#9CA3AF' : '#6B7280', marginBottom: '4px', display: 'block' };
  const valueStyle   = { fontSize: '14px', color: dm ? '#F9FAFB' : '#111827', padding: '8px 0' };
  const inputStyle   = {
  ...
  const sectionTitle = { fontSize: '16px', fontWeight: '600', color: dm ? '#F9FAFB' : '#111827', margin: '0 0 20px 0' };
  const rowStyle     = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' };
  ...
  const profileFields = [
    ...
    { label: 'Email',         name: 'email',        type: 'email', wide: true, readOnly: true },
  ...
  return (
    <div style={{ minHeight: '100vh', background: dm ? '#111827' : '#F9FAFB', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div className="flex pt-16">
        ...
        <div className="flex-1 lg:ml-64" style={{ padding: '32px 16px' }}>
          <div style={{ maxWidth: '720px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '22px', fontWeight: '700', marginBottom: '28px', color: dm ? '#F9FAFB' : '#111827' }}>
              Account Settings
            </h1>
            ...
            {!pageLoading && pageError && (
              <div style={{
                background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px',
                padding: '12px 16px', marginBottom: '16px', color: '#991B1B', fontSize: '14px',
              }}>
                {pageError} —{' '}
                <button onClick={() => window.location.reload()} style={{
                  color: '#2563EB', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px',
                }}>Retry</button>
              </div>
            )}
              ...
              <>
                {saveSuccess && (
                  <div style={{
                    ...
                    padding: '12px 16px', marginBottom: '16px', color: '#065F46', fontSize: '14px',
                  }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '28px', gap: '12px' }}>
                  <div style={{
                    ...
                    fontSize: '28px', color: '#2563EB', fontWeight: '600',
                    overflow: 'hidden',
                  }}>
                    {profile.avatar
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 40px', paddingBottom: '20px', marginBottom: '20px', borderBottom: `1px solid ${dm ? '#374151' : '#F3F4F6'}` }}>
                    {[
                      { label: 'First Name', value: profile.firstName },
                      { label: 'Last Name',  value: profile.lastName },
                    ].map(f => (
                      <div key={f.label}>
                        <div style={{ fontSize: '13px', color: dm ? '#9CA3AF' : '#6B7280', marginBottom: '4px' }}>{f.label}</div>
                        <div style={{ fontSize: '14px', color: dm ? '#F9FAFB' : '#111827' }}>{f.value || '—'}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Row 4: Department */}
                  <div>
                    <div style={{ fontSize: '13px', color: dm ? '#9CA3AF' : '#6B7280', marginBottom: '4px' }}>Department</div>
                    <div style={{ fontSize: '14px', color: dm ? '#F9FAFB' : '#111827' }}>{profile.department || '—'}</div>
                  </div>
                </div>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ ...rowStyle, marginBottom: 0 }}>
                    <span style={rowLabel}>Enable Two Factor Authentication</span>
                    <Toggle
                ...
                <div style={cardStyle}>
                  ...
                  {[
                    ...
                    <div key={key} style={{ ...rowStyle, marginBottom: i === arr.length - 1 ? 0 : '16px' }}>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{
                    ...
                    {[['Light Mode', false], ['Dark Mode', true]].map(([label, val]) => (
                      <button key={label} onClick={() => handleDarkMode(val)} style={{
                        padding: '8px 28px', border: 'none', cursor: 'pointer', fontSize: '14px',
                        background: darkMode === val ? '#2563EB' : 'transparent',
                        color: darkMode === val ? '#fff' : (dm ? '#D1D5DB' : '#374151'),
                      }}>{label}</button>
                    ))}
                  </div>
                </div>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    <button style={{
                      ...
                      border: `1px solid ${dm ? '#374151' : '#D1D5DB'}`,
                      background: 'transparent', color: dm ? '#E5E7EB' : '#374151',
      ...
      {showPasswordModal && (
        <ChangePasswordModal onClose={() => setShowPasswordModal(false)} userId={userId} />
      )}
      {showEditModal && (
        <EditProfileModal
          profile={profile}
          ...
          onSaved={(updated) => { setProfile(updated); setDraftProfile(updated); }}
        />
      )}
    </div>
  );
